...
const KIMI_TEXT_GEN_DETAIL = "Kimi does not support Git text generation.";

function makeKimiTextGeneration(_kimiSettings: KimiSettings): TextGeneration.TextGeneration["Service"] {
  const fail = (operation: string) =>
    Effect.fail(new TextGenerationError({ operation, detail: KIMI_TEXT_GEN_DETAIL }));
  return {
...
export const KimiDriver: ProviderDriver<KimiSettings, KimiDriverEnv> = {
  ...
  create: ({ instanceId, displayName, accentColor, environment, enabled, config }) =>
    Effect.gen(function* () {
      ...
      const textGeneration = makeKimiTextGeneration(effectiveConfig);

      const checkProvider = checkKimiProviderStatus(effectiveConfig).pipe(
